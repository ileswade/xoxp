...
class Board():
    ...
    def generateBoard(self, generateFor=""):
        ...
        colorX =        Color.BOLD + Color.GREEN if generateFor=="X" else Color.RED
        colorO =        Color.BOLD + Color.GREEN if generateFor=="O" else Color.RED
        colorWinline =  Color.BOLD + Color.YELLOW
        colorLines =    Color.END
        colorCounter =  Color.DARKCYAN
        buffer = ""
    ...
    def getNextPlayer(self):
        self.__playerRotation = self.__playerRotation[-1]+self.__playerRotation[0]  # This takes the first character of the string and moves it to the end and then reruns the character it just moved as it is "Their turn"
    ...
    def getFinalState(self):
        if self.__winner=="T":
            return f"{Color.BOLD}{Color.RED}Tie{Color.END} Game."
        elif self.__winner in ("XO"):
            return f"Game won by {Color.BOLD}{Color.GREEN}{self.__winner}{Color.END} along the {Color.BOLD}{Color.YELLOW}{self.__winLine}{Color.END} winline."
    ...
    def getPlay(self, player):
        while True:
            ...
            try:
                ...
                print(f"That is {Color.RED}not{Color.END} a valid character.")
                continue
            if position < 0 or position > 8: 
                print(f"That is {Color.RED}not{Color.END} a valid number (1-9).")
                continue
            if self.__gameBoard[position]!=" ":
                print(f"That space is {Color.RED}already{Color.END} played.")
                continue
            if self.__gameBoard[position]==" ": 
                return position


x=Packet.package(".","zasdfas.fs.fa.sf")
print(x)